...
angular.module("ats-pad").controller("MainController", function ($outputParser, $http, $scope, $location, $interval) {
	...
	$scope.run = function () {
		$http.post("api/" + $scope.pg.id + "/run", angular.toJson($scope.pg))
			.success(function (data, status, headers, config) {
				$scope.stdout = data.map(function (e) {
	...
	$scope.init_editor = function () {
		...
		editor.getSession().on('change', function(e) {
			$interval(function () {
				$scope.pg.files[$scope.active_file()] = editor.getValue();
			}, 1, 1);
		});
